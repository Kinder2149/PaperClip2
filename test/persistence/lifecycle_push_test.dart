...
class _SpyPortLifecycle implements CloudPersistencePort {
  int pushCount = 0;
  @override
...
void main() {
  group('Lifecycle save triggers cloud push', () {
    setUp(() {
      GamePersistenceOrchestrator.instance.resetForTesting();
    });

    test('requestLifecycleSave → push once for non-backup save', () async {
      final clock = FakeClock(DateTime.utc(2025, 1, 1, 13));
      final gs = GameState(clock: clock);
      gs.setPartieId('pid-life-1');
      final port = _SpyPortLifecycle();
      GamePersistenceOrchestrator.instance.setCloudPort(port);

      await GamePersistenceOrchestrator.instance.requestLifecycleSave(gs, reason: 'pause');
