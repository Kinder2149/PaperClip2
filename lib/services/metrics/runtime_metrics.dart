...
class RuntimeMetrics {
  static final Logger _logger = Logger.forComponent('metrics');
  ...
  static void recordTick({required int driftMs, required int durationMs}) {
    ...
    if (kDebugMode) {
      ...
      _logger.debug('[metrics] tick driftMs=$driftMs durationMs=$durationMs');
    }
  }
  ...
  static void recordOfflineApplied({required int totalSeconds, required int steps}) {
    ...
    if (kDebugMode) {
      _logger.debug('[metrics] offline applied totalSeconds=$totalSeconds steps=$steps');
    }
  }
  ...
  static void recordPause() {
    ...
    if (kDebugMode) {
      _logger.debug('[metrics] runtime pause');
    }
  }
  ...
  static void recordResume() {
    ...
    if (kDebugMode) {
      _logger.debug('[metrics] runtime resume');
    }
  }
  ...
  static void recordRecoverOffline({required int durationMs, required bool didSimulate}) {
    ...
    if (kDebugMode) {
      _logger.debug('[metrics] recoverOffline durationMs=$durationMs didSimulate=$didSimulate');
    }
  }
  ...
  static void recordAutosaveTriggered() {
    ...
    if (kDebugMode) {
      _logger.debug('[metrics] autosave triggered');
    }
  }
  ...
  static void recordAutosaveCompleted({required int durationMs, required bool success}) {
    ...
    if (kDebugMode) {
      _logger.debug('[metrics] autosave completed durationMs=$durationMs success=$success');
    }
  }
