...
class SaveMigrationService {
  ...
  static void _log(String message) {
    ...
    // Log debug optionnel supprimé (déjà loggé via _logger)
  ...
  static Future<void> compactAllSaves({bool includeBackups = true}) async {
    ...
    final metadatas = await mgr.listSaves();
    for (final meta in metadatas) {
      ...
      try {
        final loaded = await mgr.loadSave(meta.id);
        if (loaded == null) {
          continue;
        }
    ...
    try {
      ...
      for (final saveKey in savesToMigrate) {
        ...
        try {
          ...
          // Sauvegarder les données migrées via LocalSaveGameManager
          final saveGame = _createSaveGameFromMigratedData(migratedData, saveName, uuid);
          final mgr = await LocalSaveGameManager.getInstance();
          await mgr.saveGame(saveGame);
