...
class GamesServicesPlayGamesIdentityAdapter implements PlayGamesIdentityAdapter {
  ...
  Future<bool> signIn() async {
    try {
      ...
      appLogger.debug('[AUTH] [GoogleIdentity] signIn() completed, isSignedIn='+signed.toString());
      if (signed) {
        try {
          final pid = await _facade.getPlayerId();
          appLogger.debug('[AUTH] [GoogleIdentity] playerId=' + (pid ?? 'null'));
        } catch (_) {}
      }
      return signed;
    } catch (e) {
      appLogger.warn('[AUTH] [GoogleIdentity][ERR] signIn exception: ' + e.toString());
      return false;
    }
  }
