...
class SaveValidator {
  static final Logger _logger = Logger.forComponent('save-validator');
  ...
  static ValidationResult validate(
    ...
    try {
      ...
      if (kDebugMode && !_isRunningInFlutterTest) {
        _logger.debug('[SAVE] Validation de données en version '+sourceVersion+' (cible: '+validationVersion+')');
      }
      ...
      if (kDebugMode && !_isRunningInFlutterTest) {
        _logger.error('[SAVE] Erreur lors de la validation des données: $e');
      }
      return ValidationResult(
        isValid: false,
        errors: errors,
    ...
    for (final sectionName in _validationRulesV2.keys) {
      ...
      for (var fieldName in rules.keys) {
        ...
        if (!sectionData.containsKey(fieldName) && rule.containsKey('defaultValue')) {
          if (kDebugMode && !_isRunningInFlutterTest) {
            _logger.debug('[SAVE] Ajout du champ manquant '+sectionName+'.'+fieldName+' avec valeur par défaut '+rule['defaultValue'].toString());
          }
  ...
  static ValidationResult _validateGameSpecificRules(Map<String, dynamic> data) {
    ...
    try {
      ...
      if (kDebugMode && !_isRunningInFlutterTest) {
        _logger.warn('[SAVE] Erreur lors de la vérification de cohérence: $e');
      }
