...
class DataSanitizer {
  static final Logger _logger = Logger.forComponent('save-sanitizer');
  ...
  static dynamic sanitizeData(dynamic data, {bool removeNulls = false}) {
    try {
      if (data == null) {
        ...
        if (kDebugMode) {
          _logger.debug('[SAVE] Conversion de type non standard (${data.runtimeType}) en String');
        }
        return data.toString();
      }
    } catch (e) {
      if (kDebugMode) {
        _logger.warn('[SAVE] Erreur lors de la sanitisation des données: $e');
      }
      return null;
    }
  }
  ...
  static Map<String, dynamic>? sanitizeMap(Map<dynamic, dynamic> map, {bool removeNulls = false}) {
    try {
      ...
      if (kDebugMode) {
        _logger.warn('[SAVE] Erreur lors de la sanitisation de Map: $e');
      }
      return null;
    }
  }
