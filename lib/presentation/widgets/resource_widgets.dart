...
class MoneyDisplay extends StatelessWidget {
  ...
  static String formatNumber(double number, {bool isInteger = false, bool showExactCount = false}) {
    ...
    // Fonction helper pour formater avec sÃ©parateur de milliers
    ...
    if (number < 1000) {
      // Nombres infÃ©rieurs Ã  1000
      return isInteger
          ? '${number.toStringAsFixed(0)} â‚¬'
          : '${formatWithThousandSeparator(double.parse(number.toStringAsFixed(2)))} â‚¬';
    }
    ...
    // Formatage avec plus de prÃ©cision
    String formatted;
    if (simplified >= 100) {
      // Pour les nombres â‰¥ 100, on montre 3 chiffres significatifs
      formatted = simplified.toStringAsFixed(3);
      // Enlever les zÃ©ros inutiles aprÃ¨s la virgule
      if (formatted.contains('.')) {
        formatted = formatted.replaceAll(RegExp(r'\.?0+$'), '');
      }
    } else if (simplified >= 10) {
      // Pour les nombres entre 10 et 100, on montre jusqu'Ã  4 chiffres significatifs
      formatted = simplified.toStringAsFixed(3);
      if (formatted.contains('.')) {
        formatted = formatted.replaceAll(RegExp(r'\.?0+$'), '');
      }
    } else {
      // Pour les nombres < 10, on montre jusqu'Ã  4 chiffres significatifs
      formatted = simplified.toStringAsFixed(3);
      if (formatted.contains('.')) {
        formatted = formatted.replaceAll(RegExp(r'\.?0+$'), '');
      }
    ...
    return '$formatted${suffixes[index]} â‚¬';
  }
          ...
          child: Padding(
            ...
            child: Column(
              children: [
                ...
                Text(
                  '${metalStock.toStringAsFixed(1)} unitÃ©s',
                  style: TextStyle(
                    fontSize: 12,
                    color: criticalLevel
                        ? Colors.red
