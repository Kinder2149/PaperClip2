...
class SaveDataValidator {
  ...
  static ValidationResult validate(Map<String, dynamic> data) {
    ...
    if (!data.containsKey('version') || !data.containsKey('timestamp')) {
      errors.add('DonnÃ©es de base manquantes (version ou timestamp)');
      return ValidationResult(isValid: false, errors: errors);
    }

    // VÃ©rification des sections obligatoires
    if (!_validateMandatorySections(data, errors)) {
      return ValidationResult(isValid: false, errors: errors);
    }

    // VÃ©rification des rÃ¨gles pour chaque section
    for (var sectionName in _validationRules.keys) {
      ...
      // VÃ©rifier chaque champ de la section
    ...
    // Si pas d'erreurs, les donnÃ©es sont valides
  ...
  static bool _validateMandatorySections(Map<String, dynamic> data, List<String> errors) {
    ...
    if (playerData == null) {
      errors.add('DonnÃ©es du joueur invalides');
      return false;
    }
  ...
  static bool _validateField(dynamic value, Map<String, dynamic> rule, List<String> errors, String fieldPath) {
    ...
    // VÃ©rification des limites pour les nombres
    if (value is num) {
  ...
  static Future<bool> quickValidate(Map<String, dynamic> data) async {
    try {
      ...
      // VÃ©rification rapide des donnÃ©es critiques
      final playerData = data['playerManager'] as Map<String, dynamic>?;
...
class SaveGame {
  ...
  Map<String, dynamic> toJson() {
    ...
    // Ajoute les donnÃ©es du jeu Ã  la racine et dans gameData
  ...
  factory SaveGame.fromJson(Map<String, dynamic> json) {
    try {
      // Si les donnÃ©es sont dans gameData, utilise-les
      Map<String, dynamic> gameData = json['gameData'] as Map<String, dynamic>? ?? {};

      // Si les donnÃ©es sont Ã  la racine, fusionne-les avec gameData
...
class SaveManager {
  ...
  Future<String> compressSaveData(Map<String, dynamic> data) async {
    ...
    // Utiliser compute pour dÃ©placer la compression sur un autre thread
  ...
  static Future<bool> restoreFromBackup(String backupName, GameState gameState) async {
    try {
      ...
      // CrÃ©er un objet SaveGame avec les donnÃ©es actuelles du jeu
  ...
  static Future<Map<String, dynamic>?> debugSave(String name) async {
    try {
      ...
      if (savedData == null) {
        print('Aucune sauvegarde trouvÃ©e pour: $name');
        return null;
      }
  ...
  // MÃ©thode pour assurer la compatibilitÃ© avec l'ancien code
  // Ajouter cette mÃ©thode Ã  la classe SaveManager
  static Future<void> saveGameState(GameState gameState, String name) async {
    try {
      if (name.isEmpty) {
        throw SaveError('INVALID_NAME', 'Le nom de la sauvegarde ne peut pas Ãªtre vide');
      }
      ...
      if (!_quickValidate(gameData)) {
        throw SaveError('VALIDATION_ERROR', 'DonnÃ©es de base manquantes');
      }
  ...
  static Future<SaveGame?> loadGame(String name) async {
    try {
      ...
      // Si la validation est OK, crÃ©er l'objet SaveGame
  ...
  // RÃ©cupÃ©rer la derniÃ¨re sauvegarde
  ...
  static Future<List<SaveGameInfo>> listSaves() async {
    try {
      ...
      // Trier par date de sauvegarde (plus rÃ©cent d'abord)
