...
class EventManager extends ChangeNotifier implements JsonLoadable {
  ...
  Future<void> addNotification(NotificationEvent newNotification) async {
    ...
    try {
      ...
      appLogger.warn('[STATE] Erreur lors de l\'ajout de la notification: '+e.toString());
    }
  }
...
''';
  ...
  void fromJson(Map<String, dynamic> json) {
    try {
      appLogger.debug('[STATE] Loading event manager data');
      _notifications.clear();
      _unreadNotificationIds.clear();
      _events.clear();
      ...
      for (final notificationJson in notificationsJson) {
        try {
          ...
          appLogger.warn('[STATE] Erreur lors du chargement d\'une notification: '+e.toString());
        }
      }
      ...
      for (final eventJson in eventsJson) {
        try {
          ...
          appLogger.warn('[STATE] Erreur lors du chargement d\'un événement: '+e.toString());
        }
      }
      ...
      appLogger.error('[STATE] Error loading event manager: '+e.toString(), ctx: {
