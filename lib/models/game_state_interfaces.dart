...
mixin GameStateMarket on ChangeNotifier {
  ...
  void initializeMarket() {
    marketManager = MarketManager(
      // Lecture de la pause via l'état maître (GameState) si disponible
      pauseReader: () {
        if (this is GameState) {
          final gs = this as GameState;
          return gs.isPaused;
        }
        return false;
      },
...
mixin GameStateSave on ChangeNotifier {
  Future<void> saveGame() async {
    try {
      ...
      if (kDebugMode) {
        appLogger.warn('[STATE] Error saving game: '+e.toString());
      }
    }
  }
  ...
  Future<void> loadGame() async {
    try {
      ...
      if (kDebugMode) {
        appLogger.warn('[STATE] Error loading game: '+e.toString());
      }
    }
  }
