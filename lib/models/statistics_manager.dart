    ...
    try {
      ...
      if (json.containsKey('gameStartTime')) {
        try {
          ...
          if (kDebugMode) {
            appLogger.warn('[STATE] Erreur de conversion de la date de début de jeu: '+e.toString());
          }
        }
      }
    } catch (e) {
      if (kDebugMode) {
        appLogger.warn('[STATE] Erreur lors du chargement des données statistiques: '+e.toString());
      }
    }
  }
