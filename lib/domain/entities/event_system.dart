...
/// DÃ©finition des prioritÃ©s de notification
...
/// Classe reprÃ©sentant un Ã©vÃ©nement de notification
class NotificationEvent {
  ...
  // MÃ©thode pour incrÃ©menter le compteur d'occurrences
...
/// Classe reprÃ©sentant un Ã©vÃ©nement du jeu
class GameEvent {
  final EventType type;
  final String title;
  final String description;
...
class EventManager with  ChangeNotifier {
  ...
  final Map<UnlockableFeature, UnlockDetails> _unlockDetailsMap = {
    // Ajoutez les dÃ©tails de dÃ©verrouillage ici
    UnlockableFeature.MANUAL_PRODUCTION: UnlockDetails(
      ...
      benefits: ['Production immÃ©diate', 'Gain d\'expÃ©rience'],
      tips: ['Maintenez un stock de mÃ©tal'],
  ...
  Future<void> addNotification(NotificationEvent newNotification) async {
    ...
    // VÃ©rifier si une notification similaire a Ã©tÃ© montrÃ©e rÃ©cemment
    ...
    if (lastShownTime != null &&
        ...
        newNotification.type != EventType.LEVEL_UP) {  // Ne pas appliquer l'intervalle minimum pour les montÃ©es de niveau
      return;
    }

    // Mettre Ã  jour le temps de derniÃ¨re apparition
    _lastShownTimes[newNotification.groupId ?? newNotification.title] = DateTime.now();

    try {
      // Pour les notifications de niveau, toujours ajouter une nouvelle entrÃ©e
  ...
  void clearNotifications() {
    ...
    notificationStream.value = null;
    markAllAsRead(); // Utiliser la nouvelle mÃ©thode au lieu de _unreadNotificationIds.clear()
    notifyListeners();
  }
  ...
  void markAllAsRead() {
    ...
    // Notifier les Ã©couteurs du changement
    notifyListeners();
  }


  // VÃ©rifier si une notification est non lue
  ...
  void addEvent(EventType type,
    ...
    if (type == EventType.LEVEL_UP &&
      ...
      if (unlockDetails != null) {
        additionalData = {
          ...additionalData,
          'FonctionnalitÃ©': unlockDetails.name,
...
âœ¨ Avantages :
${unlockDetails.benefits.map((b) => 'â€¢ $b').join('\n')}

ðŸ’¡ Conseils :
${unlockDetails.tips.map((t) => 'â€¢ $t').join('\n')}
''';
      }
    }
  ...
  final Map<MarketEvent, CrisisGuide> _crisisGuides = {
    MarketEvent.MARKET_CRASH: CrisisGuide(
      title: "Guide : GÃ©rer un Krach du MarchÃ©",
      description: "Le marchÃ© s'effondre, voici comment rÃ©agir",
      steps: [
        "1. RÃ©duisez votre production de 50%",
    ...
    MarketEvent.PRICE_WAR: CrisisGuide(
      title: "Guide : Survivre Ã  une Guerre des Prix",
      description: "Les concurrents baissent agressivement leurs prix",
      steps: [
        "1. Maintenez des prix compÃ©titifs",
        "2. Focalisez sur l'efficacitÃ©",
        "3. AmÃ©liorez votre marketing",
    ...
    MarketEvent.DEMAND_SPIKE: CrisisGuide(
      ...
      steps: [
        ...
        "2. Ajustez vos prix Ã  la hausse",
    ...
    MarketEvent.QUALITY_CONCERNS: CrisisGuide(
      title: "Guide : RÃ©soudre les ProblÃ¨mes de QualitÃ©",
      description: "La qualitÃ© de vos produits est remise en question",
      steps: [
        "1. Investissez dans la qualitÃ©",
        "2. Baissez temporairement les prix",
        "3. AmÃ©liorez votre rÃ©putation",
        "4. Communiquez sur vos amÃ©liorations"
      ],
      icon: Icons.warning,
