...
import '../../presentation/screens/competitive_result_screen.dart';
...
class GameState with ChangeNotifier with ChangeNotifier with ChangeNotifier extends ChangeNotifier {
  ...
  // Mode de jeu (infini ou compÃ©titif)
  ...
  void _initializeManagers() {
    if (!_isInitialized) {
      // Ã‰tape 1 : CrÃ©ation des managers
      _createManagers();

      // Ã‰tape 2 : Configuration et dÃ©marrage
      _configureAndStart();
  ...
  void _createManagers() {
    try {
      ...
      print('Erreur lors de la crÃ©ation des managers: $e');
      rethrow;
    }
  }
  ...
  // Gestionnaire de timers centralisÃ©
  ...
  // Timers
  // Ajouter aprÃ¨s les propriÃ©tÃ©s existantes (vers ligne 43)
  ...
  void handleCompetitiveGameEnd() {
    ...
    // Calculer les mÃ©triques de la partie compÃ©titive
    ...
    // DÃ©verrouiller les succÃ¨s compÃ©titifs si nÃ©cessaire
    _checkCompetitiveAchievements(competitiveScore);

    // Afficher le rÃ©sultat si on a un contexte
    if (_context != null) {
  ...
  // MÃ©thode pour dÃ©marrer une nouvelle partie compÃ©titive
  void _startNewCompetitiveGame(BuildContext context) {
    final gameName = 'CompÃ©tition_${DateTime.now().day}_${DateTime.now().month}_${DateTime.now().hour}${DateTime.now().minute}';
    startNewGame(gameName, mode: GameMode.COMPETITIVE).then((_) {
      // Retourner Ã  l'Ã©cran principal avec la nouvelle partie
  ...
  // MÃ©thode pour calculer le score compÃ©titif
  int calculateCompetitiveScore() {
    ...
    print('Score compÃ©titif calculÃ©: $finalScore (Prod: $productionScore, Argent: $moneyScore, Niveau: $levelScore, EfficacitÃ©: $efficiencyBonus, Temps: $timeMultiplier)');

    return finalScore;
  }

  // MÃ©thode pour calculer l'efficacitÃ© (ratio trombones/mÃ©tal)
  double calculateEfficiencyRating() {
    // Obtenir la consommation totale de mÃ©tal depuis les statistiques
    double totalMetalUsed = _statistics.getTotalMetalUsed();
    if (totalMetalUsed <= 0) return 1.0;

    // Ratio trombones/mÃ©tal (ajustÃ© pour Ãªtre significatif)
    return _totalPaperclipsProduced / totalMetalUsed;
  }

  // VÃ©rification des rÃ©alisations (achievements) compÃ©titifs
  void _checkCompetitiveAchievements(int score) {
    final gamesServices = GamesServicesController())));

    // VÃ©rifier les seuils de score
    if (score >= 100000) {
    ...
    // VÃ©rifier le temps de jeu (parties rapides)
    ...
    if (!await gamesServices.isSignedIn()) {
      if (_context != null) {
        ScaffoldMessenger.of(_context!).showSnackBar(
          const SnackBar(
            content: Text('Connectez-vous Ã  Google Play Games pour synchroniser'),
  ...
  void enterCrisisMode() {
    ...
    print("DÃ©but de la transition vers le mode crise");

    // GÃ©rer spÃ©cifiquement le mode compÃ©titif
    if (_gameMode == GameMode.COMPETITIVE) {
      ...
      EventManager().addEvent(
          ...
          "Mode Crise ActivÃ©",
          description: "Fin de partie compÃ©titive : plus de mÃ©tal disponible !",
      ...
      // Sauvegarder l'Ã©tat avant d'afficher les rÃ©sultats
      saveOnImportantEvent();

      // GÃ©rer la fin de partie compÃ©titive
      ...
      showDialog(
        ...
        builder: (context) => MetalCrisisDialog(
          onTransitionComplete: () {
            // Activer le mode crise aprÃ¨s la fermeture du dialogue
            _isInCrisisMode = true;
            _crisisStartTime = DateTime.now();
            ...
            EventManager().addEvent(
                EventType.CRISIS_MODE,
                "Mode Crise ActivÃ©",
                description: "Adaptation nÃ©cessaire : plus de mÃ©tal disponible !",
                importance: EventImportance.CRITICAL,
                additionalData: {
                  'timestamp': _crisisStartTime!.toIso8601String(),
                  'marketMetalStock': marketManager.marketMetalStock,
            ...
            // Activer les nouvelles fonctionnalitÃ©s
            _unlockCrisisFeatures();

            saveOnImportantEvent(); // Sauvegarder l'Ã©tat aprÃ¨s la transition
            notifyListeners();
          },
        ),
      );
  ...
  void _unlockCrisisFeatures() {
    // Supprimer les rÃ©fÃ©rences au recyclage
    ...
    EventManager().addEvent(
        ...
        description: "Vous pouvez maintenant produire votre propre mÃ©tal !",
        importance: EventImportance.CRITICAL
    );
  ...
  void reset() {
    ...
    // Ne pas rÃ©initialiser les managers, juste leurs Ã©tats
  ...
  void _startTimers() {
    ...
    // Timer du marchÃ© - Ajout de cette partie
  ...
  Map<String, dynamic> prepareGameData() {
    ...
    // Ajout des donnÃ©es des managers
    try {
      ...
      // Debug logs
      print('PrepareGameData - Sauvegarde des donnÃ©es:');
      print('Mode de jeu: ${_gameMode == GameMode.INFINITE ? "Infini" : "CompÃ©titif"}');
      print('Mode crise actif: ${_isInCrisisMode}');
      print('DÃ©but de la crise: ${_crisisStartTime?.toIso8601String()}');
      print('Transition complÃ¨te: $_crisisTransitionComplete');
      print('DonnÃ©es joueur: ${baseData['playerManager']}');
      print('DonnÃ©es marchÃ©: ${baseData['marketManager']}');
  ...
  void processProduction() {
    ...
    // Nouveau calcul d'efficacitÃ© avec 11% par niveau et plafond 85%
    ...
    // MÃ©tal nÃ©cessaire par trombone avec efficacitÃ©
    double metalPerClip = GameConstants.METAL_PER_PAPERCLIP * efficiencyBonus;

    // Nombre maximum de trombones possibles avec le mÃ©tal disponible
    int maxPossibleClips = (playerManager.metal / metalPerClip).floor();

    // Production effective (limitÃ©e par le mÃ©tal disponible)
    ...
    if (actualProduction > 0) {
      ...
      double metalSaved = actualProduction * GameConstants.METAL_PER_PAPERCLIP * reduction; // DÃ©placÃ© ici
      ...
      // ExpÃ©rience pour la production automatique
  ...
  void checkResourceCrisis() {
    if (marketManager.marketMetalStock <= 0 && !_isInCrisisMode) {
      print("DÃ©clenchement de la crise - Stock Ã©puisÃ©");

      EventManager().addEvent(
          EventType.RESOURCE_DEPLETION,
          "Stock Mondial Ã‰puisÃ©",
          description: "Les rÃ©serves mondiales de mÃ©tal sont Ã©puisÃ©es.\nDe nouveaux moyens de production doivent Ãªtre trouvÃ©s !",
          importance: EventImportance.CRITICAL,
  ...
  bool validateCrisisTransition() {
    if (!_isInCrisisMode) {
      print("Erreur: Mode crise non activÃ©");
      return false;
    }

    if (!_crisisTransitionComplete) {
      print("Erreur: Transition non terminÃ©e");
      return false;
    }
    saveOnImportantEvent();
  ...
  // Actions du jeu
  void buyMetal() {
    print('Tentative d\'achat de mÃ©tal'); // Debug
    ...
    print('Prix: $metalPrice, QuantitÃ©: $amount'); // Debug

    // Le joueur peut payer et stocker le mÃ©tal
    playerManager.updateMoney(playerManager.money - metalPrice);
    playerManager.updateMetal(playerManager.metal + amount);
    marketManager.updateMarketStock(-amount);  // Important: le signe nÃ©gatif

    print('Achat effectuÃ© - Nouveau stock marchÃ©: ${marketManager.marketMetalStock}'); // Debug

    if (marketManager.marketMetalStock <= 0) {
      print('Stock Ã©puisÃ© - DÃ©clenchement mode crise'); // Debug
      enterCrisisMode();
    }
    _statistics.updateEconomics(
  ...
  bool _canBuyMetal() {
    ...
    return playerManager.money >= metalPrice &&
        ...
        marketManager.marketMetalStock >= GameConstants.METAL_PACK_AMOUNT;  // Ajout de cette vÃ©rification
  }
  ...
  void producePaperclip() {
    if (player.consumeMetal(GameConstants.METAL_PER_PAPERCLIP)) {
      ...
      // Mettre Ã  jour le leaderboard tous les 100 trombones
    ...
    for (var feature in newFeatures) {
      switch (feature) {
        case UnlockableFeature.MANUAL_PRODUCTION:
          _showUnlockNotification('Production manuelle dÃ©bloquÃ©e !');
          break;
        case UnlockableFeature.MARKET_SALES:
          _showUnlockNotification('Ventes dÃ©bloquÃ©es !');
          break;
        ...
        case UnlockableFeature.METAL_PURCHASE:
          _showUnlockNotification('Achat de mÃ©tal dÃ©bloquÃ© !');
          break;
        case UnlockableFeature.MARKET_SCREEN:
          _showUnlockNotification('Ã‰cran de marchÃ© dÃ©bloquÃ© !');
          break;
        case UnlockableFeature.UPGRADES:
          _showUnlockNotification('AmÃ©liorations disponibles !');
          break;
      }
    }

    // Mise Ã  jour de l'achievement progressif
    final gamesServices = GamesServicesController())));
  ...
  void _handleMissionCompleted(Mission mission) {
    ...
    EventManager().addEvent(
        ...
        "Mission accomplie !",
        description: "${mission.title} - ${mission.experienceReward} XP gagnÃ©s",
        importance: EventImportance.MEDIUM
    );
  }
  // Dans GameState, ajoutez un logger pour le debug
  // Mise Ã  jour pour prendre en charge la synchronisation cloud
  Future<void> startNewGame(String name, {GameMode mode = GameMode.INFINITE, bool syncToCloud = false}) async {
    try {
      ...
      // Si mode compÃ©titif, initialiser le timer
      if (_gameMode == GameMode.COMPETITIVE) {
        _competitiveStartTime = DateTime.now();
      }

      // RÃ©initialiser l'Ã©tat si dÃ©jÃ  initialisÃ©
      if (_isInitialized) {
        reset();
      }
      ...
      // CrÃ©er un nouvel objet SaveGame
      ...
      // Synchroniser avec le cloud si demandÃ©
      if (syncToCloud) {
        final gamesServices = GamesServicesController())));
        if (await gamesServices.isSignedIn()) {
  ...
  Future<void> loadGame(String name, {String? cloudId}) async {
    try {
      ...
      if (cloudId != null) {
        ...
        if (saveGame == null) throw SaveError('NOT_FOUND', 'Sauvegarde non trouvÃ©e');
      }
      ...
      // Charger le temps de dÃ©part en mode compÃ©titif
      ...
      // Charger les donnÃ©es globales avec conversion sÃ©curisÃ©e
    ...
    try {
      ...
      if (await gamesServices.isSignedIn()) {
        ...
        if (selectedSave != null && _context != null) {
          ...
          // Charger la partie sÃ©lectionnÃ©e
      ...
      print('Erreur lors de l\'affichage du sÃ©lecteur cloud: $e');
      if (_context != null) {
        ScaffoldMessenger.of(_context!).showSnackBar(
          SnackBar(
  ...
  void showLeaderboard() {
    // Ajouter le paramÃ¨tre manquant
  ...
  Future<void> saveGame(String name, {bool syncToCloud = true}) async {
    if (!_isInitialized) {
      throw SaveError('NOT_INITIALIZED', 'Le jeu n\'est pas initialisÃ©');
    }

    try {
      // PrÃ©paration des donnÃ©es de jeu
      final gameData = prepareGameData();

      // CrÃ©ation de l'objet SaveGame
      final saveData = SaveGame(
        ...
        version: GameConstants.VERSION,
        gameMode: _gameMode,
        isSyncedWithCloud: false,  // Sera mis Ã  jour aprÃ¨s la synchronisation
      );
      ...
      // Synchronisation avec le cloud si demandÃ© et connectÃ©
      if (syncToCloud) {
        final gamesServices = GamesServicesController())));
        if (await gamesServices.isSignedIn()) {
          ...
          if (success && _context != null) {
            ScaffoldMessenger.of(_context!).showSnackBar(
              const SnackBar(
                content: Text('Partie sauvegardÃ©e et synchronisÃ©e avec le cloud'),
  ...
  Future<void> saveOnImportantEvent() async {
    ...
    try {
      ...
      print('Erreur lors de la sauvegarde Ã©vÃ©nementielle: $e');
    }
  }
    ...
    try {
      ...
      for (var backup in backups) {
        try {
          ...
          print('Restauration rÃ©ussie depuis le backup: ${backup.name}');
          return;
        } catch (e) {
          print('Ã‰chec de la restauration depuis ${backup.name}: $e');
          continue;
        }
      }
    } catch (e) {
      print('Erreur lors de la vÃ©rification des backups: $e');
    }
  }
  ...
  void _showUnlockNotification(String message) {
    EventManager().addNotification(
      NotificationEvent(
        title: 'Nouveau DÃ©blocage !',
        description: message,
  ...
  void toggleCrisisInterface() {
    ...
    EventManager().addInterfaceTransitionEvent(_showingCrisisView);
