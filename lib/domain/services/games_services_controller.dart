...
class GamesServicesController {
  ...
  Future<bool> signIn() async {
    try {
      // Diagnostic de dÃ©bogage
      print("Tentative de connexion Ã  Google Play Games Services");
      ...
      if (signedIn) {
        _isSignedIn = true;
        print("Connexion aux services de jeu rÃ©ussie");
        return true;
      } else {
        print("La connexion n'a pas abouti, mais aucune erreur n'a Ã©tÃ© lancÃ©e");

        // Tenter Ã  nouveau aprÃ¨s une courte pause, cela peut parfois aider
        ...
        if (secondAttempt) {
          print("Connexion rÃ©ussie Ã  la seconde tentative");
          return true;
        }

        print("Ã‰chec mÃªme aprÃ¨s seconde tentative");
        print("VÃ©rifiez dans Google Play Console que les empreintes SHA-1 suivantes sont enregistrÃ©es:");
        print("DÃ©bogage: 94:95:FD:94:32:6F:9D:6C:1A:64:99:91:9E:41:47:7C:FB:84:F7:54");
      ...
      try {
        ...
        // Ignorer si l'erreur ne peut pas Ãªtre enregistrÃ©e
      }
  ...
  // RÃ©cupÃ©rer la liste des sauvegardes cloud
  Future<List<sm.SaveGameInfo>> getCloudSaves() async {
    if (!_isSignedIn) return [];

    try {
      // Utiliser CloudSaveManager pour rÃ©cupÃ©rer les sauvegardes
      final cloudSaveManager = CloudSaveManager();
  ...
  // Afficher une interface permettant de sÃ©lectionner une sauvegarde
  // Fonction adaptÃ©e pour games_services 4.0.3
  Future<sm.SaveGame?> showSaveSelector() async {
    if (!_isSignedIn) return null;

    try {
      // Comme games_services 4.0.3 n'a pas de showSavedGamesUI,
      // nous crÃ©ons notre propre interface ou utilisons une alternative

      // RÃ©cupÃ©rer les sauvegardes cloud
      final cloudSaves = await getCloudSaves();

      // Si aucune sauvegarde, retourner null
      if (cloudSaves.isEmpty) return null;

      // Simuler une sÃ©lection (dans une vraie application, vous afficheriez une UI)
      // Par dÃ©faut, prenons la sauvegarde la plus rÃ©cente
      cloudSaves.sort((a, b) => b.timestamp.compareTo(a.timestamp));
      final selectedSave = cloudSaves.first;

      // Charger la sauvegarde sÃ©lectionnÃ©e
  ...
  // DÃ©bloquer un succÃ¨s compÃ©titif
  Future<void> unlockCompetitiveAchievement(
    ...
    try {
      ...
      // DÃ©terminer l'ID du succÃ¨s Ã  dÃ©bloquer
      switch (achievement) {
  ...
  Future<void> submitGeneralScore(int paperclips, int money, int playTime) async {
    ...
    try {
      // Score gÃ©nÃ©ral basÃ© sur les trombones, l'argent et le temps de jeu
...
// MÃ©thode pour soumettre le score de production
...
// MÃ©thode pour soumettre le score bancaire
  ...
  // Cette mÃ©thode est simplifiÃ©e car getLeaderboardScores n'existe pas dans la version 4.0.3
  ...
  int _calculateGeneralScore(int paperclips, int money, int playTime) {
    ...
    // Score de base basÃ© sur les trombones et l'argent
    int baseScore = (paperclips * PAPERCLIP_WEIGHT) + (money * MONEY_WEIGHT);

    // Bonus d'efficacitÃ© temporelle (plus le temps est court, plus le bonus est grand)
  ...
  // MÃ©thode pour afficher les achievements
  ...
  // MÃ©thode pour afficher les leaderboards
  Future<void> showLeaderboard({required String leaderboardID, bool friendsOnly = false}) async {
    try {
      final signedIn = await GamesServices.isSignedIn;
      if (!signedIn) {
        print("L'utilisateur n'est pas connectÃ©, tentative de connexion...");
        await signIn();
      }
...
// MÃ©thode pour afficher le classement de production
...
// MÃ©thode pour afficher le classement bancaire
  ...
  // MÃ©thode pour mettre Ã  jour tous les classements d'un coup
  Future<void> updateAllLeaderboards(GameState gameState) async {
    if (!_isSignedIn) return;

    try {
      // On utilise catchError pour chaque opÃ©ration individuelle
      await submitGeneralScore(
        ...
        debugPrint('Error submitting general score: $e');
        return null; // Retourne null mais continue l'exÃ©cution
      });
      ...
      // Si une erreur se produit quand mÃªme
  ...
  Future<GooglePlayerInfo?> getCurrentPlayerInfo() async {
    ...
    try {
      ...
      // Essayons d'utiliser une approche diffÃ©rente
      return GooglePlayerInfo(
        id: 'player_id',
        displayName: 'Joueur connectÃ©', // Nommer gÃ©nÃ©riquement pour l'instant
        iconImageUrl: null,
      );
    } catch (e, stackTrace) {
      print("Erreur lors de la rÃ©cupÃ©ration des infos du joueur: $e");
      FirebaseCrashlytics.instance.recordError(e, stackTrace);
      return null;
    }
  }
  ...
  Future<bool> switchAccount() async {
    try {
      // Pour forcer la sÃ©lection d'un compte, nous pouvons nous dÃ©connecter temporairement
      // (cela n'est pas directement supportÃ© par l'API, mais nous pouvons simuler)
      _isSignedIn = false;
      // Supprimez cette ligne : notifyListeners();

      // Attendre un peu pour s'assurer que le changement d'Ã©tat est pris en compte
      await Future.delayed(Duration(milliseconds: 500));

      // RÃ©initialiser toute instance/Ã©tat stockÃ© en cache
